  ...
  return <>
    ...
    <main id="main" tabIndex={-1}>
      <section id="home" className="hero shell" aria-labelledby="hero-title">
        ...
        <div className="playground"><div className="stage-top"><span className="eyebrow">A little world of what I do</span><span className="stage-star" aria-hidden="true">✳</span></div><div className="scene-wrap"><Suspense fallback={<FallbackArt chapter={chapter}/>}><Sculpture chapter={chapter} paused={paused||reduced} reset={reset} onReady={setSceneReady} fallback={<FallbackArt chapter={chapter}/>}/></Suspense><span className="scene-word" aria-hidden="true">{chapters[chapter].name.toLowerCase()}.</span></div><div className="scene-toolbar"><span>{sceneReady ? "Drag to rotate · Try a chapter below" : "Choose a chapter to explore my story"}</span>{sceneReady && <div><button className="icon-button" aria-label="Reset sculpture rotation" onClick={()=>setReset(v=>v+1)}><RotateCcw size={15}/></button><button className="icon-button" aria-label={paused||reduced?'Play sculpture animation':'Pause sculpture animation'} aria-pressed={paused||reduced} onClick={()=>{setPaused(!(paused||reduced));setReduced(false)}}>{paused||reduced?<Play size={15}/>:<Pause size={15}/>}</button></div>}</div><div className="chapter-tabs" role="tablist" aria-label="Explore my story">{chapters.map((c,i)=><button key={c.name} ref={el=>tabs.current[i]=el} id={'chapter-'+i} role="tab" aria-selected={chapter===i} aria-controls="chapter-panel" tabIndex={chapter===i?0:-1} onClick={()=>setChapter(i)} onKeyDown={e=>tabKey(e,i)}><span>0{i+1}</span>{c.name}<ArrowUpRight size={16}/></button>)}</div><div id="chapter-panel" role="tabpanel" aria-labelledby={'chapter-'+chapter} className="chapter-panel" tabIndex={0}><p className="eyebrow">{chapters[chapter].tag}</p><h2>{chapters[chapter].title}</h2><p>{chapters[chapter].text}</p></div></div>
      </section>
      ...
      <section id="contact" className="contact" aria-labelledby="contact-title"><div className="shell"><p className="eyebrow">03 — YOUR NEXT IDEA?</p><div className="contact-heading"><h2 id="contact-title">Let’s make<br/>something <span>click.</span></h2><a className="contact-circle" href={socials.email.url} aria-label="Email Gajendra"><ArrowUpRight/></a></div><div className="contact-bottom"><div><p>Have an idea, a question, or just a hello?</p><a className="email-link" href={socials.email.url}>{profile.email}<ArrowUpRight size={20}/></a></div><div className="social-links">{[socials.linkedin,socials.github,socials.youtube].map(s=><a key={s.label} href={s.url} target="_blank" rel="noreferrer">{s.label}<ArrowUpRight size={16}/></a>)}</div></div></div></section>
