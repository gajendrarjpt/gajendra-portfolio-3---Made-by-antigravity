...
// Structured Incident Case Studies (Simulated Engineering Post-Mortem Playbooks)
export const incidentCaseStudies = [
  {
    id: 'core-loop-stp',
    index: '01',
    severity: 'SIMULATED SCENARIO · P1',
    title: 'Enterprise Core Switching Loop & Broadcast Storm Resolution',
    category: 'SWITCHING & HIGH AVAILABILITY PLAYBOOK',
    environment: 'Reference Topology: Cisco Catalyst 9300 Stack · Rapid-PVST+ · Multi-VLAN Campus',
    problem: 'Conceptual failure mode: Unmanaged edge switch connected into access port without BPDU Guard, causing root bridge usurpation and broadcast circulation.',
    impact: 'Workstations and VoIP devices experience gateway unreachable errors due to switch TCAM buffer saturation and CPU spikes.',
    detection: 'NMS high-CPU threshold alert triggered; Syslog indicates rapid MAC address flapping between trunk and access ports.',
    investigation: 'Console session shows continuous spanning-tree topology change notifications (TCN) arriving at 2-second intervals.',
    diagnosis: 'Absence of Spanning-Tree BPDU Guard on edge access ports allowed an external BPDU to initiate a topology recalculation loop.',
    resolution: 'Isolate and disable the looped access port; enforce Spanning-Tree BPDU Guard and Root Guard on all access edge ports; lock core stack priority at 4096.',
    verification: 'Verify CPU returns to baseline; confirm MAC address flap notifications cease; validate spanning-tree topology state is stable and forwarding.',
    outcome: 'Eliminated broadcast loop vulnerability by establishing gold-template BPDU Guard and PortFast standards across access layer.'
  },
  {
    id: 'bgp-wan-failover',
    index: '02',
    severity: 'SIMULATED SCENARIO · P2',
    title: 'BGP WAN Flapping & Sub-Second Fast-Reroute Optimization',
    category: 'ROUTING & WAN INFRASTRUCTURE PLAYBOOK',
    environment: 'Reference Topology: Cisco ISR 4451 · Dual eBGP Providers (AS65001 / AS65002)',
    problem: 'Conceptual failure mode: Intermittent WAN link micro-flapping causing continuous BGP route dampening and table recalculation cycles.',
    impact: 'Outbound applications experience session drops and high latency during standard BGP hold-time expiration windows.',
    detection: 'Syslog registers repeated BGP neighbor adjacency state transitions (ESTABLISHED to IDLE/ACTIVE).',
    investigation: 'Examined BGP routing table; identified that carrier degradation did not trigger immediate interface down signals, delaying standard hold-timer failover.',
    diagnosis: 'Standard BGP keepalive timers (60s/180s) are insufficient for sub-second failure detection on degraded physical circuits.',
    resolution: 'Configure Bidirectional Forwarding Detection (BFD) on eBGP peering sessions; implement BGP conditional route-maps for deterministic failover to secondary provider.',
    verification: 'Simulate interface degradation; verify BFD detects path loss within 300ms and redirects traffic with minimal TCP retransmissions.',
    outcome: 'Achieved sub-second automated WAN failover using BFD and route-map policies to insulate enterprise traffic from brownout events.'
  },
  {
    ...
    category: 'NETWORK SECURITY & FIREWALL PLAYBOOK',
    environment: 'Reference Topology: Palo Alto PA-3220 (Headquarters) ↔ FortiGate (Branch Office)',
    problem: 'Conceptual failure mode: Site-to-site IPsec VPN tunnel drops periodically at Phase 2 Security Association (SA) rekey intervals.',
    impact: 'Branch office users lose connectivity to centralized data center services during cryptographic renegotiation.',
    detection: 'Syslog event correlation shows IKE Phase 2 SA deletion and negotiation timeout errors at predictable time intervals.',
    investigation: 'Captured IKE Phase 1 and Phase 2 negotiation debug logs simultaneously on both firewall vendor appliances.',
    diagnosis: 'Parameter mismatch in Phase 2 Lifetime and Perfect Forward Secrecy (PFS) DH Group between Palo Alto PAN-OS and Fortinet FortiOS crypto profiles.',
    resolution: 'Align Phase 2 crypto proposals (AES-256-GCM, SHA-256, DH Group 14); enable Dead Peer Detection (DPD) with standardized keepalive intervals on both gateways.',
    verification: 'Trigger manual Phase 2 SA rekeying; verify continuous ICMP connectivity during crypto renegotiation with zero packet drop.',
    outcome: 'Standardized multi-vendor IPsec configuration template ensuring continuous tunnel uptime across heterogeneous firewall environments.'
  },
  {
    id: 'wireshark-tcp-window',
    index: '04',
    severity: 'SIMULATED SCENARIO · P3',
    title: 'Deep Packet Inspection: TCP Receive Window Exhaustion Diagnostics',
    category: 'TELEMETRY & PACKET ANALYSIS PLAYBOOK',
    environment: 'Reference Topology: Wireshark SPAN Port Mirroring · Cisco Catalyst Core · Application Server',
    problem: 'Conceptual failure mode: Severe application file download slowness despite core network utilization remaining well below bandwidth capacity.',
    impact: 'Users report extended file transfer times, while network monitoring graphs show healthy link throughput and zero dropped frames.',
    detection: 'Standard SNMP interface metrics show nominal line rates; escalated for deep packet capture and protocol analysis.',
    investigation: 'Configured SPAN port mirror on server uplink interface; captured frame stream during active transfer for Wireshark dissection.',
    diagnosis: 'Wireshark Expert Info flagged repeated TCP ZeroWindow and Window Full warnings generated by the client, indicating endpoint TCP buffer exhaustion rather than network transit congestion.',
    resolution: 'Adjusted client-side TCP receive window auto-tuning parameters and updated legacy network adapter drivers to enable TCP Window Scaling (RFC 7323).',
    verification: 'Recaptured packet stream in Wireshark; confirmed TCP window size expanded dynamically, eliminating ZeroWindow stalls.',
    outcome: 'Methodical packet-level analysis provided empirical evidence to isolate root cause to endpoint TCP stack, clearing network infrastructure.'
  }
];
